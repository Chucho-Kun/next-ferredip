// scripts/redownload-truper-completo.mjs
//
// Re-descarga TODAS las filas listadas en fotos-truper/_reporte_faltantes.csv
// (326 filas, sin filtrar por estado previo) usando la URL ya presente en la
// columna `url` de ese CSV, y las guarda en fotos-truper/{id}.jpg.
//
// Se usa cuando fotos-truper/ perdio los .jpg pero conserva los reportes CSV:
// en vez de reconstruir URLs desde `clave` (que puede tener casos especiales
// como "/" o espacios ya resueltos a mano en corridas previas), se reutiliza
// tal cual la URL que ya funciono/fallo en la corrida anterior.
import { writeFile, readFile, mkdir } from "node:fs/promises";
import { existsSync } from "node:fs";
import path from "node:path";

const OUTPUT_DIR = "fotos-truper";
const INPUT_CSV = path.join(OUTPUT_DIR, "_reporte_faltantes.csv");
...
async function main() {
  ...
  const todas = await leerLista();
  const pendientes = args.limit ? todas.slice(0, args.limit) : todas;

  console.log(`Filas a procesar desde ${INPUT_CSV}: ${todas.length}`);
  if (args.dryRun) {
