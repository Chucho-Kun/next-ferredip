// scripts/redownload-truper-ok.mjs
//
// Re-descarga las filas con estado "ok" de fotos-truper/_reporte.csv (la
// corrida original, ~1843 filas) usando la URL ya presente en la columna
// `url`, y las guarda en fotos-truper/{id}.jpg. Se usa junto con
// redownload-truper-completo.mjs (que cubre las filas de _reporte_faltantes.csv)
// para reconstruir fotos-truper/ completo cuando se perdieron los .jpg.
import { writeFile, readFile, mkdir } from "node:fs/promises";
import { existsSync } from "node:fs";
import path from "node:path";

const OUTPUT_DIR = "fotos-truper";
const INPUT_CSV = path.join(OUTPUT_DIR, "_reporte.csv");
const REPORT_CSV = path.join(OUTPUT_DIR, "_reporte_ok.csv");
...
async function leerOk() {
  const contenido = await readFile(INPUT_CSV, "utf8");
  const [, ...lineas] = contenido.split("\n").filter((l) => l.trim() !== "");
  return lineas
    .map(parseCsvLine)
    .map(([id, clave, url, estado]) => ({ id, clave, url, estado }))
    .filter((row) => row.estado === "ok" && row.url && row.url.trim() !== "");
}
...
async function main() {
  ...
  const todas = await leerOk();
  const pendientes = args.limit ? todas.slice(0, args.limit) : todas;

  console.log(`Filas con estado "ok" en ${INPUT_CSV}: ${todas.length}`);
  if (args.dryRun) {
